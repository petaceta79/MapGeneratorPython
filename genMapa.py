...
def eliminarRodeadas(mapa, posicion):
    if EstaRodeadaSiONo(mapa, posicion) and random.random() < porcentageEliminarRodeadas:
        return False
    else:
        return True
    
# Ampliar el mapa
def ampliarMapa(mapa, NumeroCasillas, tiposDeHabitaciones):
    if(NumeroCasillas+1 <= (len(mapa)*len(mapa))): # Comprobar que no pida mas casillas de las posibles
        ...
        for e in posicionDeCasillas(mapa):
            if not(eliminarRodeadas(mapa, e)):
                mapa[e[0]][e[1]] = 0
    return mapa

# Empieza codigo 
tamanoDelMapa = 15
numeroDeHabitacionesIntervalo = [70,80]
numeroDeHabitaciones = random.randint(numeroDeHabitacionesIntervalo[0],numeroDeHabitacionesIntervalo[1])
tiposDeHabitaciones = [2,3,4,5,6]
porcentageEliminarRodeadas = 0.8

mapa = generarArrayMapa(tamanoDelMapa-1)
mapa[int(len(mapa)/2)][int(len(mapa[0])/2)] = 1 # La primera habitacion en el centro

mapa = ampliarMapa(mapa, numeroDeHabitaciones-1, tiposDeHabitaciones)
